...
DEFAULT_SETTINGS = {
    ...
    "tool_permissions": {
        ...
        "switch_project": True,
        "list_projects": True,
        "list_network_servers": True,
        "list_server_files": True,
        "download_server_file": True,
        "read_emails": True,
        "send_email": True,
        "list_processes": True,
        "kill_process": True,
    ...
    "network_servers": [], # List of {name, host, share, username, password}
...
async def startup_event():
    ...
    asyncio.create_task(monitor_system_stats())

@app.get("/status")
async def status():
    return {"status": "running", "service": "A.P.A Backend"}
...
async def start_audio(sid, data=None):
    ...
    def on_audio_data(data_bytes):
        # Ensure we only process actual bytes
    ...
    # Callback to send Widget updates to frontend
    def on_widget_update(data):
        asyncio.create_task(sio.emit('widget_update', data))

    # Callback to move UI widgets
    def on_widget_move(data):
        asyncio.create_task(sio.emit('widget_move', data))
    ...
    try:
        ...
        # Load saved network servers
        saved_servers = SETTINGS.get("network_servers", [])
        if saved_servers and audio_loop.network_agent:
            print(f"[SERVER] Loading {len(saved_servers)} saved network servers...")
            for s in saved_servers:
                audio_loop.network_agent.add_server(
...
async def monitor_system_stats():
    """Background task to emit system stats periodically."""
    print("[SERVER] Starting System Monitor Loop")
    while True:
        try:
            ...
            # Get GPU info if available (Windows)
            gpu_usage = 0
            try:
                import subprocess
                ...
                pass  # Fallback to 0 if no NVIDIA GPU or command fails
            ...
            print(f"[SERVER] System Monitor Error: {e}")
...
async def discover_printers(sid):
    ...
    if not audio_loop or not audio_loop.printer_agent:
        ...
        if saved_printers:
            ...
            await sio.emit('status', {'msg': "Connect to A.P.A to enable printer discovery"})
            return
...
@sio.event
async def configure_network_server(sid, data):
    """Adds or updates a network server configuration."""
    print(f"Received configure_network_server: {data}")
    try:
        name = data.get("name")
        host = data.get("host")
        ...
        if not all([name, host, share]):
            await sio.emit('error', {'msg': "Missing required fields: name, host, share"})
            return
            
        if audio_loop and audio_loop.network_agent:
            audio_loop.network_agent.add_server(name, host, share, username, password)
            
            # Save to settings
            server_config = {"name": name, "host": host, "share": share, "username": username, "password": password}
            existing = SETTINGS.get("network_servers", [])
            # Update if exists, else append
            updated = False
            for i, s in enumerate(existing):
                if s.get("name") == name or s.get("host") == host:
                    existing[i] = server_config
                    updated = True
                    break
            if not updated:
                existing.append(server_config)
            SETTINGS["network_servers"] = existing
            save_settings()
            
            await sio.emit('status', {'msg': f"Network server '{name}' configured"})
        else:
            await sio.emit('error', {'msg': "Network Agent not available"})
    except Exception as e:
        print(f"Error configuring network server: {e}")
        await sio.emit('error', {'msg': f"Failed to configure server: {str(e)}"})

@sio.event
async def configure_email(sid, data):
    """Updates email configuration."""
    print(f"Received configure_email request")
    try:
        email_config = SETTINGS.get("email_config", {})
        ...
        if audio_loop and audio_loop.email_agent:
            audio_loop.email_agent.email_address = email_config.get("email_address", "")
            audio_loop.email_agent.password = email_config.get("password", "")
            audio_loop.email_agent.imap_server = email_config.get("imap_server", "imap.gmail.com")
            audio_loop.email_agent.smtp_server = email_config.get("smtp_server", "smtp.gmail.com")
            
            conn_status = audio_loop.email_agent.connect()
            if "error" in conn_status:
                await sio.emit('error', {'msg': conn_status["error"]})
            else:
                await sio.emit('status', {'msg': "Email configured successfully"})
        else:
            await sio.emit('error', {'msg': "Email Agent not available"})
    except Exception as e:
        print(f"Error configuring email: {e}")
        await sio.emit('error', {'msg': f"Failed to configure email: {str(e)}"})

@sio.event
async def fetch_today_emails(sid):
    print("[SERVER] Fetching today's emails...")
    try:
        if not audio_loop or not audio_loop.email_agent:
            await sio.emit('email_error', {'msg': "Email Agent not available."})
            return
            
        # Add a 10-second timeout to prevent hanging
        result = await asyncio.wait_for(audio_loop.email_agent.fetch_today_emails(), timeout=10.0)
        
        if "error" in result:
            await sio.emit('email_error', {'msg': result["error"]})
            # Push error widget
            if audio_loop and hasattr(audio_loop, '_push_widget'):
                audio_loop._push_widget("error", {
            ...
            if audio_loop and hasattr(audio_loop, '_push_widget') and "emails" in result:
                emails_data = []
                ...
                audio_loop._push_widget("email_summary", {
                    "count": result["count"],
                    "emails": emails_data
                })
    except asyncio.TimeoutError:
        print("[SERVER] Email fetch timed out.")
        await sio.emit('email_error', {'msg': "Email fetch timed out. Check your connection or credentials."})
    except Exception as e:
        print(f"Error fetching emails: {e}")
        await sio.emit('email_error', {'msg': f"Failed to fetch emails: {str(e)}"})

@sio.event
async def desktop_hand_move(sid, data):
    """Move the system mouse cursor via Windows API (Instant)."""
    x = int(data.get('x', 0))
    y = int(data.get('y', 0))
    ctypes.windll.user32.SetCursorPos(x, y)

@sio.event
async def desktop_hand_click(sid, data=None):
    """Simulate a mouse click via Windows API."""
